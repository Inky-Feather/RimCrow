<template>
  <div class="max-w-24 mt-0.5 flex items-center h-min py-0 px-0.5 mr-1 rounded-md text-xs border transition-colors select-none group"
       :class="tagStyle">
    <!-- 排除条件用前缀 - 标识，和输入语法保持一致。 -->
    <span v-if="tag.exclude" class="text-accent-danger font-bold">-</span>
    <!-- 规则 token 显示字段名；纯文本 token 不显示 key。 -->
    <span v-if="displayKey" class="opacity-70">{{ displayKey }}:</span>
    <!-- 值区域：双击进入编辑，长文本通过 tooltip 查看完整内容。 -->
    <div v-tooltip="displayValue" class="flex-1 font-medium truncate min-w-0 relative">
    ...
    <!-- 删除按钮默认收起，hover 时展开，避免 token 较多时占用过多横向空间。 -->
    <button @click.stop="$emit('remove')"
      ...
      <svg class="shrink-0 size-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
...
// originalKey 保留用户输入的别名；没有别名时回退到真实字段名。
const displayKey = computed(() => props.tag.originalKey || props.tag.displayKey || props.tag.key || '')
const displayValue = computed(() => String(props.tag.displayValue ?? props.tag.value ?? ''))
...
const tagStyle = computed(() => {
  ...
  // 排除条件优先使用危险色；纯文本条件使用中性色；字段规则按 key 稳定映射颜色。
  ...
  const theme = colorPalette[hashString(String(props.tag.key)) % colorPalette.length]
  return `${theme.bg} ${theme.border} ${theme.text}`
})

watch(() => props.isEditing, (newVal) => {
  if (!newVal) return
  // 每次进入编辑都从当前展示值同步，避免复用上一次未保存的输入。
