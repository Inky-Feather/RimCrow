<template>
  <div class="space-y-3">
    <div class="flex flex-wrap items-center gap-2">
      <span class="text-xs font-bold uppercase tracking-wide text-text-dim/75">批量处理</span>
      <label v-for="option in strategyOptions" :key="option.value" class="rounded-full flex items-center border px-2 py-1 text-xs"
        :class="strategy === option.value ? 'border-accent-primary/35 bg-accent-primary/10 text-accent-primary' : 'border-text-main/10 bg-black/15 text-text-dim'" >
        <input class="mr-1 accent-accent-primary" type="radio" :checked="strategy === option.value" @change="strategy = option.value" >
        {{ option.label }}
      </label>
    </div>

    <div v-for="row in rows" :key="row.archive_key" class="rounded-xl border border-text-main/10 bg-black/15 p-3" >
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="text-sm font-bold text-text-main">{{ row.name || row.archive_key }}</div>
        <span class="text-[0.68rem] text-text-dim/75">
      ...
      <div class="mt-3 grid grid-cols-3 gap-3">
        ...
        <div v-if="row.mode === 'overwrite'" class="col-span-2 text-xs text-text-dim/75">
          <CommonSelect v-model="row.target_profile_id" label="覆盖到" :options="buildConflictOptions(row)" />
        </div>

        <div v-else-if="row.mode === 'create'" class="col-span-2 text-xs text-text-dim/75">
          <CommonSelect v-model="row.game_install_path" label="要使用的游戏目录" :options="availableInstallOptions" />
        </div>

        <div v-else class="col-span-2 rounded-lg border border-text-main/8 bg-black/10 px-3 py-2 text-xs text-text-dim/75">
          这项将跳过。
        </div>
      </div>
    </div>
