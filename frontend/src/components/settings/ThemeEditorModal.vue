  ...
  <Teleport to="body">
    <transition name="panel-fade">
      <div v-if="isOpen" ref="panelRef"
        class="fixed z-140 flex max-h-1/2 max-w-1/4 flex-col overflow-hidden rounded-3xl border border-accent-primary/24 bg-bg-deep/96 shadow-[0_18px_70px_rgba(0,0,0,0.62)] backdrop-blur-xl"
        :style="panelStyle" >
        ...
        <header class="relative z-10 flex cursor-move select-none items-center justify-between gap-3 border-b border-text-main/8 px-4 py-3"
        ...
        <div class="relative z-10 flex-1 overflow-y-auto p-3 custom-scrollbar">
          ...
          <section v-for="group in THEME_TOKEN_GROUPS" :key="group.key" class="mt-2 rounded-2xl border border-text-main/10 bg-text-main/[0.025] p-2.5">
            ...
            <div class="grid gap-1" :style="{ gridTemplateColumns: groupGridTemplate(group.key) }">
              <div v-for="token in group.tokens" :key="`${group.key}-${token.key}`" class="rounded-xl bg-black/10 px-2 py-0.5">
                <div class="flex items-start gap-2.5">
                  <button type="button" class="relative mt-0.5 size-7.5 shrink-0 overflow-hidden rounded-md ring-2 ring-text-main/55 shadow-sm transition-transform hover:scale-105"
                ...
                <p v-if="showColorValue" class="truncate text-[0.65rem] text-text-dim/70">{{ draft.tokens[group.key][token.key] }}</p>
              </div>
            </div>
          </section>
        </div>

        <footer class="relative z-10 flex items-center justify-between gap-3 border-t border-text-main/8 bg-black/15 px-4 py-3">
          ...
          <div class="flex shrink-0 gap-2">
            ...
            <button class="rounded-xl bg-accent-primary px-4 py-1.5 text-sm font-black text-black transition-all hover:bg-accent-primary/85"
              @click="handleSave"
            >保存</button>
          </div>
        </footer>
