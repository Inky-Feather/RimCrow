    ...
    <header class="relative z-10 flex items-start justify-between gap-2 border-b border-border-base/10 px-3 pt-2.5 pb-1">
      <div class="min-w-0">
        <div class="flex items-center gap-1.5">
          <h3 class="truncate text-sm font-black text-text-main">{{ t('ui.dependency_graph.manager.title', '依赖线管理') }}</h3>
        ...
        <p class="mt-0.5 text-[0.68rem] text-text-dim">
          {{ t('ui.dependency_graph.manager.summary', '可见 {visible} 条，已隐藏 {hidden} 条。', { visible: visibleCount, hidden: hiddenGroups.length }) }}
        </p>
        <p v-if="focusSourceId" class="mt-0.5 text-[0.62rem] font-bold text-accent-special">
          {{ t('ui.dependency_graph.manager.focus_hint', '当前仅绘制 1 条，右键绘图区可恢复。') }}
        </p>
      </div>
      <div class="flex shrink-0 items-center gap-1.5">
        <button class="modal-close-button"
          :aria-label="t('ui.dependency_graph.manager.close', '关闭依赖线管理')"
          @click="$emit('close')">
    ...
    <div class="relative z-10 space-y-2 px-2 py-2">
      <div class="flex items-center">
        ...
        <label class="flex-1 flex items-center gap-1.5 rounded-xl border border-border-base/10 bg-bg-inset/40 px-2.5 py-1.5 text-[0.68rem] text-text-dim focus-within:border-accent-primary/25 focus-within:text-text-main">
          ...
          <input v-model.trim="searchQuery" type="text" class="min-w-0 flex-1 bg-transparent text-[0.68rem] text-text-main outline-none"
            :placeholder="t('ui.dependency_graph.manager.search_placeholder', '搜索依赖源名称或包名')">
        </label>
      ...
      <div class="relative z-10 flex items-center justify-between gap-2 text-[0.65rem] text-text-dim">
        ...
        <button v-if="activePage === 'hidden' && hiddenGroups.length > 0"
          ...
          {{ t('ui.dependency_graph.manager.restore_all', '全部恢复') }}
        </button>
      </div>
    </div>
      ...
      <div v-if="activeGroups.length > 0" class="grid grid-cols-2 gap-1.5">
        <button v-for="group in activeGroups" :key="`${activePage}-${group.id}`"
          ...
          <span class="min-w-0 flex-1">
            ...
            <span v-if="activePage === 'hidden' && !group.inCurrentList" class="mt-0.5 block truncate text-[0.6rem] leading-tight text-accent-special">{{ t('ui.dependency_graph.manager.historical_hidden_item', '历史隐藏项') }}</span>
          </span>
          <span class="shrink-0 text-right text-[0.62rem] leading-tight text-text-dim">
            {{ activePage === 'hidden' ? group.childCountLabel : t('ui.dependency_graph.manager.child_count', '{count}项', { count: group.childCount }) }}
          </span>
        </button>
      </div>
...
const pages = computed(() => ([
  { key: 'visible', label: t('ui.dependency_graph.manager.tab_visible', '可见'), count: filteredCurrentGroups.value.length },
  { key: 'hidden', label: t('ui.dependency_graph.manager.tab_hidden', '隐藏'), count: filteredHiddenGroups.value.length },
]))
...
const activePageMeta = computed(() => (
  activePage.value === 'hidden'
    ? {
      tip: t('ui.dependency_graph.manager.hidden_tip', '点击任意隐藏项可立即恢复显示。'),
      emptyText: t('ui.dependency_graph.manager.hidden_empty', '当前没有符合搜索条件的隐藏依赖线。'),
    }
    : {
      tip: t('ui.dependency_graph.manager.visible_tip', '点击任意可见项可直接隐藏该依赖线。'),
      emptyText: t('ui.dependency_graph.manager.visible_empty', '当前没有符合搜索条件的可见依赖线。'),
    }
))
</script>
