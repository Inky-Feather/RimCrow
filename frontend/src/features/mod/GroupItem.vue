<template>
  <div :style="{ '--rgb-components': hexToRgb(groupData.color) }"
    ...
    <div @click="toggle" :class="['list-none select-none px-1 flex text-text-dim hover:text-text-main items-center justify-between gap-0.5 rounded-lg font-medium',
      ...
      <div v-tooltip="t('tooltip.group_item.move', '移动')" class="select-trigger cursor-move p-1 text-text-dim hover:text-text-main hover:scale-130 transition-all">
      ...
      <!-- 颜色选择与展开显示 -->
      <div @click.stop v-tooltip="t('tooltip.group_item.change_color', '改变颜色')" class="no-drag relative inline-flex items-center justify-center text-text-main hover:text-transparent transition-all">
        <!--
      ...
      <span class="flex items-center">
        <button @mousedown.prevent @click.stop="openExportDialog" v-tooltip="t('tooltip.group_item.package_export', '打包导出分组模组')" :class="`rounded-lg p-1 hover:bg-bg-overlay/10 cursor-pointer text-text-dim text-xs font-bold shadow-lg hover:shadow-bg-deep/50 transition-all`">
          <Package class="size-4.5 hover:text-accent-special" />
        </button>
        <!-- 编辑/保存按钮 -->
        <button @mousedown.prevent @click.stop="toggleEditName" v-tooltip="t('tooltip.group_item.edit_name', '编辑分组名称')" :class="`rounded-lg p-1 hover:bg-bg-overlay/10 cursor-pointer text-text-dim text-xs font-bold shadow-lg hover:shadow-bg-deep/50 transition-all`">
        ...
        <!-- 删除按钮 -->
        <button @click.stop="deleteGroup($event)" v-tooltip="t('tooltip.group_item.delete', '删除分组')" :class="`rounded-lg p-1 hover:bg-bg-overlay/10 cursor-pointer
...
const openExportDialog = () => {
  appStore.openCustomModExportDialog({
    title: t('dialog.group_item.export.title', '导出分组模组: {name}', { name: props.groupData?.name || t('ui.group_list.unnamed_group', '未命名分组') }),
    description: t('dialog.group_item.export.description', '可按需附带依赖、联锁项和语言包。'),
    modIds: [...groupModIds.value],
    summary: t('dialog.group_item.export.summary', '分组内共 {count} 个模组。', { count: groupModIds.value.length }),
  })
}
// 切换展开状态
const toggle = () => {
