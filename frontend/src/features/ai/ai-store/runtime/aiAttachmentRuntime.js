...
const formatTokenEstimateText = (tokenInfo = null) => {
  ...
  return t('ai.attachments.token_estimate', '约 {count}k token', { count: (estimated / 1000).toFixed(1) })
}
...
export const buildAttachmentDisplayMeta = (attachment, getAttachmentDefinition) => {
  ...
  if (!attachment) {
    return { summary: t('ai.attachments.attachment', '附件'), detail: '' }
  }
  ...
  if (summary) {
    return {
      summary: t('ai.attachments.summary_prefix', '附件: {summary}', { summary }),
      detail,
    }
  }
  ...
  return {
    summary: t('ai.attachments.summary_prefix', '附件: {summary}', { summary: definition?.label || t('ai.attachments.context', '上下文') }),
    detail,
  }
}
...
export const buildDiagnosisContextAttachmentDraft = ({
  ...
  snapshot: {
    summary: normalizeText(summaryText) || (
      isGlobalSummary
        ? t('ai.attachments.log_summary', '{source}摘要', { source: normalizeText(sourceLabel, t('ai.attachments.log', '日志')) })
        : t('ai.attachments.selected_logs', '{count} 条已选{source}', { count: Number(selectedLogCount || 0), source: normalizeText(sourceLabel, t('ai.attachments.log', '日志')) })
    ),
