    ...
    size="page" :z-index="120" accent="special" panel-class="border-accent-special/25" content-class="h-full flex flex-col" @close="closeModal" >
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-4">
          <div v-if="reviewTasks.length === 0" class="h-full flex items-center justify-center text-sm text-text-dim">
	            {{ t('dialog.mod_alias_review.empty', '当前没有待检阅的批量结果') }}
          </div>
          ...
          <div v-for="group in reviewTasks" :key="group.taskId" class="modal-section overflow-hidden" >
            ...
            <div class="toolbar-surface px-5 py-4">
              <div class="flex items-start justify-between gap-4">
                <div class="min-w-0">
	                  <div class="text-base font-black text-text-main">{{ group.title || t('dialog.mod_alias_review.default_task_title', '模组别名生成任务') }}</div>
                  <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[0.7rem] text-text-dim">
	                    <span>{{ t('dialog.mod_alias_review.task_id', '任务ID: {taskId}', { taskId: group.taskId }) }}</span>
	                    <span>{{ t('dialog.mod_alias_review.created_at', '创建时间: {time}', { time: formatTime(group.meta?.created_at || group.createdAt) }) }}</span>
	                    <span>{{ t('dialog.mod_alias_review.attempt_count', '轮次: {current}/{max}', { current: Number(group.meta?.attempt_count || 0), max: Number(group.meta?.max_attempts || 3) }) }}</span>
	                    <span>{{ t('dialog.mod_alias_review.input_count', '输入: {count}', { count: Number(group.meta?.input_total || group.inputPackageIds?.length || 0) }) }}</span>
	                    <span>{{ t('dialog.mod_alias_review.success_count', '成功: {count}', { count: Number(group.meta?.resolved_count || countSucceeded(group.items)) }) }}</span>
	                    <span v-if="countFailed(group.items) > 0">{{ t('dialog.mod_alias_review.failed_count', '失败: {count}', { count: countFailed(group.items) }) }}</span>
                  </div>
                </div>
            ...
            <div class="p-4 space-y-3">
              <div v-for="(item, index) in group.items" :key="`${group.taskId}:${item.package_id}`" class="group rounded-xl border overflow-hidden transition-all flex flex-col relative"
                ...
                <div v-if="item._failed" class="absolute bottom-0 right-0 px-3 py-1 bg-accent-warn/20 text-accent-warn text-[0.6rem] z-50 font-bold rounded-tl-lg rounded-br-xl border-t border-l border-accent-warn/30">
	                  {{ t('dialog.mod_alias_review.generation_failed_badge', '生成失败，请手填或重试') }}
                </div>
                ...
                <div class="flex flex-1 p-3 gap-4 relative">
                  <div class="w-4/12 flex gap-3 pr-4 border-r border-border-base/10">
                    ...
                    <div class="flex-1 min-w-0 flex flex-col">
	                      <div class="text-[0.65rem] text-text-dim font-bold tracking-wider mb-0.5">{{ t('dialog.mod_alias_review.original_info', '原始信息') }}</div>
                      ...
                      <div class="text-[0.6rem] text-text-dim mt-1 line-clamp-5 leading-relaxed" :title="getMod(item.package_id)?.description">
	                        {{ getMod(item.package_id)?.description || t('dialog.mod_alias_review.no_description', '暂无描述信息...') }}
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="w-8/12 flex flex-col gap-1">
                    <div class="flex items-center gap-2">
                      <label class="w-12 shrink-0 text-[0.65rem] text-right uppercase text-accent-special font-bold tracking-widest" :class="{ 'text-accent-warn': !item.alias_name }">
	                        {{ t('dialog.mod_alias_review.ai_alias', 'AI 别名') }}
                      </label>
	                      <input v-model="item.alias_name" :placeholder="t('dialog.mod_alias_review.alias_placeholder', '请输入或点击重试生成...')"
                    ...
                    <div class="flex items-start gap-2 flex-1">
                      <label class="w-12 shrink-0 text-[0.65rem] text-right uppercase text-accent-special font-bold tracking-widest mt-2" :class="{ 'text-accent-warn': !item.notes }">
	                        {{ t('dialog.mod_alias_review.ai_notes', 'AI 备注') }}
                      </label>
	                      <textarea v-model="item.notes" :placeholder="t('dialog.mod_alias_review.notes_placeholder', '请输入或点击重新生成...')"
    ...
    <template #footer>
        <div class="flex items-center justify-between gap-4">
          <div class="text-xs text-text-dim">
	            {{ t('dialog.mod_alias_review.footer_tip', '未确认应用前，结果会一直保留在这里，可随时回来继续检阅。') }}
          </div>
...
const taskStatusLabel = (group) => {
	  ...
	  if (status === 'running') return t('common.status.processing', '处理中')
	  if (status === 'pending') return t('common.status.pending', '排队中')
	  if (status === 'cancelled') return t('common.status.cancelled', '已取消')
	  if (status === 'error' || status === 'failed') return t('common.status.failed', '失败')
	  return t('dialog.mod_alias_review.status.pending_review', '待检阅')
}
    ...
    if (result) {
      aiStore.updateModAliasReviewTaskItem(taskId, item.package_id, {
        ...
        _error: '',
	      ...
	      toast.success(t('toast.mod_alias_review.regenerated', '[{name}] 已重新生成', { name: mod.name || item.package_id }))
	    } else {
	      toast.error(t('toast.mod_alias_review.regenerate_failed', '重新生成失败'))
    }
  } catch (error) {
