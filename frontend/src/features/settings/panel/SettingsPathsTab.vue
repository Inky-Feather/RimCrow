<template>
              <section class="animate-in fade-in slide-in-from-right-4">
                <div class="flex items-center justify-between mb-6">
                  <h3 class="text-lg font-bold text-text-main">{{ t('ui.settings.paths.title', '环境与路径配置') }}
                    <label v-tooltip="t('tooltip.settings.paths.edit_current_profile', '此处会直接修改当前环境的路径配置')" class="text-text-dim ml-1 cursor-help italic underline hover:text-text-main">?</label>
                    <label class="ml-5 text-xs py-0.5 px-2 text-accent-cool bg-accent-cool/20 rounded-md" v-tooltip="t('tooltip.settings.paths.current_profile', '当前环境：{name}\n说明：{description}', { name: profileStore?.currentProfile?.name || '', description: profileStore?.currentProfile?.description || '' })">
                  ...
                  <button @click="handleAutoDetect" :disabled="isPending('auto-detect')" :class="isPending('auto-detect') ? 'app-action-disabled' : ''"
                    v-tooltip="isPending('auto-detect') ? t('tooltip.settings.paths.auto_detect_loading', '正在自动搜索路径') : t('tooltip.settings.paths.auto_detect', '尝试通过系统和 Steam 记录自动搜索路径')" class="inline-flex items-center gap-1 px-3 py-1 bg-accent-success/10 hover:bg-accent-success/20 border border-accent-success/30 rounded text-xs font-bold text-accent-success transition-all">
                    <LoaderCircle v-if="isPending('auto-detect')" class="size-3 animate-spin" />
                    {{ isPending('auto-detect') ? t('ui.settings.paths.searching', '搜索中') : t('ui.settings.paths.auto_detect', '自动搜索路径') }}
                  </button>
                </div>
                <div class="grid gap-6">
                  <CommonPathInput :label="t('ui.settings.paths.game_install_path', '游戏安装目录')" v-model="formData.game_install_path" @browse="handleGameBrowse"
                    :check="formData.check_info?.game_install_path"
                    :description="t('ui.settings.paths.game_install_path_desc', '游戏安装目录即游戏主程序所在的目录，不同系统位置不同，Windows下默认安装目录一般位于：\nC:/Program Files (x86)/Steam/steamapps/common/RimWorld')"
                    @blur="checkPath('game_install_path', formData.game_install_path)"/>
                  <CommonPathInput :label="t('ui.settings.paths.user_data_path', '用户数据目录')" v-model="formData.user_data_path" @browse="handleBrowse('user_data_path')"
                    :check="formData.check_info?.user_data_path"
                    :description="t('ui.settings.paths.user_data_path_desc', '用户数据目录即游戏配置、存档和日志所在的目录，不同系统位置不同，可使用自动搜索或手动选择。\nWindows下默认数据目录一般位于：\nC:/Users/{用户名}/AppData/LocalLow/Ludeon Studios/RimWorld by Ludeon Studios')"
                  ...
                  <CommonPathInput :label="t('ui.settings.paths.workshop_mods_path', '创意工坊目录')" v-model="formData.workshop_mods_path" @browse="handleBrowse('workshop_mods_path')"
                  ...
                  <div class="modal-section grid grid-cols-1 gap-2 p-3">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                      <CommonSwitch :label="t('ui.settings.paths.prefer_steam_launch', '优先使用 Steam 启动')" :disabled="isPending('steam-launch-check')"
                        :model-value="formData.prefer_steam_launch" :description="t('ui.settings.paths.prefer_steam_launch_desc', '开启后，管理器会优先通过 Steam 启动当前环境，并直接使用 Steam 中的创意工坊内容。路径不完整时会提醒，但仍可手动开启。')"
                  ...
                  <div class="modal-section grid grid-cols-2 gap-2 p-3">
                    <h3 class="col-span-2 text-sm font-bold ml-1 text-text-main">{{ t('ui.settings.paths.manager_mods', '管理器模组') }}</h3>
                    <CommonPathInput class=" col-span-2" :label="t('ui.settings.paths.self_mods_path', '管理器下载模组路径')" :check="formData.check_info?.self_mods_path"
                      :description="t('ui.settings.paths.self_mods_path_desc', '由管理器下载的模组所在的目录，可自定义位置，如果将其设为游戏本地模组路径，请关闭该使用开关。')"
                      v-model="formData.self_mods_path" @browse="handleBrowse('self_mods_path')" @blur="checkPath('self_mods_path', formData.self_mods_path)"
                    />
                    <CommonSwitch :label="t('ui.settings.paths.use_manager_mods', '使用管理器模组')" v-model="formData.use_self_mods" :description="t('ui.settings.paths.use_manager_mods_desc', '开启后将通过链接方式自动为游戏加载管理器Mod。')" />
                    <CommonSwitch :label="t('ui.settings.paths.move_old_self_mods', '改变路径时移动模组')" v-model="formData.move_old_self_mods" :description="t('ui.settings.paths.move_old_self_mods_desc', '开启后，修改路径时会将原有模组移动到新路径；不开启则保留原有的文件结构。')" />
                    <CommonSwitch class="col-span-1" :label="t('ui.settings.paths.auto_check_manager_mod_updates', '自动检查管理器模组更新')" v-model="formData.enable_auto_steamcmd_mod_update_check" :description="t('ui.settings.paths.auto_check_manager_mod_updates_desc', '按设定间隔检查管理器模组目录中由 SteamCMD 下载的工坊模组和 Git 仓库订阅模组更新。')" />
                    <div class="col-span-1 grid grid-cols-2 gap-3 items-end">
                      <CommonNumber class="col-span-1" :label="t('ui.settings.paths.check_interval_days', '检查间隔（天）')" v-model="formData.steamcmd_mod_update_check_interval_days" :step="1" :min="1" :max="365" />
                      <button @click="handleCheckSteamcmdMods" :disabled="isPending('steamcmd-mods')" :class="isPending('steamcmd-mods') ? 'app-action-disabled' : ''"
                        ...
                        {{ isPending('steamcmd-mods') ? t('ui.settings.paths.checking_updates', '检查中') : t('ui.settings.paths.check_updates', '检查更新') }}
                      </button>
                    </div>
                  </div>
                  <div class="modal-section grid grid-cols-2 gap-2 p-3">
                    <h3 class="col-span-2 text-sm font-bold ml-1 text-text-main">{{ t('ui.settings.paths.load_order_dialog_paths', '排序导入/导出 起始选择窗口配置') }}</h3>
                    <CommonSelect class="col-span-1" :label="t('ui.settings.paths.import_start_dir', '导入起始目录')" v-model="formData.load_order_import_dir_mode"
                      :description="t('ui.settings.paths.import_start_dir_desc', '控制“导入加载序列”文件选择器的选择窗口初始目录。默认模式始终使用当前环境用户数据目录下的 ModLists；记忆模式使用上次成功导入的目录；自定义模式使用下方固定目录。')"
                      :options="LOAD_ORDER_DIR_MODE_OPTIONS"
                    />
                    <CommonSelect class="col-span-1" :label="t('ui.settings.paths.export_start_dir', '导出起始目录')" v-model="formData.load_order_export_dir_mode"
                      :description="t('ui.settings.paths.export_start_dir_desc', '控制“导出加载序列”文件选择器的选择窗口初始目录。默认模式保持当前环境备份目录的 other 子目录；记忆模式使用上次成功导出的目录；自定义模式使用下方固定目录。')"
                      :options="LOAD_ORDER_DIR_MODE_OPTIONS"
                    />
                    <CommonPathInput v-if="formData.load_order_import_dir_mode === 'custom'" class="col-span-2" :label="t('ui.settings.paths.custom_import_start_dir', '自定义导入起始目录')" v-model="formData.load_order_import_custom_path"
                      :check="formData.check_info?.load_order_import_custom_path" :description="t('ui.settings.paths.custom_start_dir_desc', '仅在导入目录模式为“自定义”时生效；若路径无效，运行时会自动回退到默认目录。')"
                    ...
                    />
                    <CommonPathInput v-if="formData.load_order_export_dir_mode === 'custom'" class="col-span-2" :label="t('ui.settings.paths.custom_export_start_dir', '自定义导出起始目录')" v-model="formData.load_order_export_custom_path"
...
const LOAD_ORDER_DIR_MODE_OPTIONS = computed(() => [
  ...
  { label: t('ui.settings.paths.dir_mode.remember', '记忆'), value: 'remember' },
  { label: t('ui.settings.paths.dir_mode.custom', '自定义'), value: 'custom' },
])
