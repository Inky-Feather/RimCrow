              ...
              <section data-tour="settings-ai-section" class="animate-in fade-in slide-in-from-right-4">
                <div class="mb-6 flex items-center justify-between">
                  <h3 class="text-lg font-bold text-text-main flex items-center gap-2">
                    {{ t('ui.settings.ai.title', '人工智能') }}
                    <span class="px-2 py-0.5 rounded bg-accent-special/20 text-accent-special text-xs font-black uppercase">{{ t('ui.settings.ai.experimental', '实验性') }}</span>
                  </h3>
                  ...
                  <button v-if="formData.ai.enabled" @click="appStore.uiState.showAIDefinitionManager = true"
                    ...
                    <Drama class="size-4" /> {{ t('ui.settings.ai.definition_manager', 'AI 定义管理') }}
                  </button>
                </div>
                <div class="space-y-6">
                  <div data-tour="settings-ai-enable">
                    <CommonSwitch :label="t('ui.settings.ai.enable', '启用 AI 功能')" v-model="formData.ai.enabled" :description="t('ui.settings.ai.enable_desc', '用于日志分析、模组说明和 AI 助手对话等功能。')" />
                  </div>
                  <div v-if="formData.ai.enabled" class="space-y-6 animate-in slide-in-from-top-2">
                    ...
                    <div data-tour="settings-ai-connection" class="p-4 rounded-xl bg-bg-overlay/5 border border-border-base/10 space-y-5">
                      <div class="grid grid-cols-2 gap-3">
                        <!-- 厂商/协议选择 -->
                        <CommonSelect :label="t('ui.settings.ai.provider', '接口协议')" :description="t('ui.settings.ai.provider_desc', '大多数中转服务、本地运行时、国产模型平台都优先兼容 OpenAI-compatible 接口。只有目标服务没有稳定的 OpenAI-compatible API 时，才建议切换到原生协议。')"
                          v-model="formData.ai.provider" :options="currentAiProviders" @change="handleProviderChange"/>
                        <!-- 模型选择 (带刷新动作) -->
                        <div class="relative flex items-end gap-2">
                          <div class="flex-1">
                            <!-- 加上 editable 允许用户手输未被探测到的模型名 -->
                            <CommonSelect :label="t('ui.settings.ai.model', '模型')" editable v-model="formData.ai.model" :options="currentAiModels" 
                              :placeholder="t('ui.settings.ai.model_placeholder', '下拉选择或手动输入模型名称')" @visible-change="(val) => val && fetchAiModels({ silent: true })"/>
                          </div>
                          <!-- 对于自定义模式，提供显式的刷新按钮让用户主动拉取 -->
                          <button @click="fetchAiModels({ forceRefresh: true, warnOnEmpty: true, silent: false })" v-tooltip="t('tooltip.settings.ai.refresh_models', '重新获取模型列表')"
                    ...
                    <div data-tour="settings-ai-advanced" class="">
                      <div class="grid grid-cols-3 gap-2">
                        <CommonNumber :label="t('ui.settings.ai.max_concurrency', '最大并发数')" v-model="formData.ai.max_concurrency" :step="1" :min="1" :max="100" :description="t('ui.settings.ai.max_concurrency_desc', '同时发出的请求数量。大多数情况下设为 3 到 5 就够了。')" />
                        <CommonNumber :label="t('ui.settings.ai.temperature', '输出随机性')" v-model="formData.ai.temperature" :step="0.1" :min="0" :max="2.0" :description="t('ui.settings.ai.temperature_desc', '值越低越稳定，值越高越发散。一般用 0.7 左右即可。')" />
                        <CommonNumber :label="t('ui.settings.ai.context_window_tokens', '上下文窗口')" v-model="formData.ai.context_window_tokens" :step="1024" :min="0"
                          :description="t('ui.settings.ai.context_window_tokens_desc', '模型总上下文窗口。0 表示按模型名自动预设；本地模型如果服务端限制了上下文，建议填实际值。')" />
                        <CommonNumber :label="t('ui.settings.ai.max_input_tokens', '最大输入预算')" v-model="formData.ai.max_input_tokens" :step="1024" :min="0"
                          :description="t('ui.settings.ai.max_input_tokens_desc', '日志、附件、批量任务最多喂给模型的输入预算。0 表示自动按上下文窗口扣除输出预算。')" />
                        <CommonNumber :label="t('ui.settings.ai.max_output_tokens', '最大输出预算')" v-model="formData.ai.max_output_tokens" :step="512" :min="0"
                          :description="t('ui.settings.ai.max_output_tokens_desc', '单次回复的输出保护阀。0 表示按模型预设自动控制；只有排查成本、延迟或长回复截断时才需要手动设置。')" />
                        <CommonSelect v-if="formData.ai.provider === 'openai_compatible'"
                          :label="t('ui.settings.ai.endpoint_mode', '接口模式')" v-model="formData.ai.endpoint_mode"
                          :description="t('ui.settings.ai.endpoint_mode_desc', '用于指定 OpenAI-compatible 接口应走哪一类 endpoint。`Auto` 会根据模型能力和请求结构自动选择更稳妥的路径；只有在排查特定中转服务或本地运行时兼容问题时，才建议手动切换。')"
                          :options="endpointModeOptions" />
                      </div>

                      <!-- 测试区 -->
                      <div data-tour="settings-ai-test" class="pt-4 flex gap-3">
                        <CommonInput :label="t('ui.settings.ai.test_prompt', '测试内容')" class="flex-1" v-model="testPrompt" :placeholder="t('ui.settings.ai.test_prompt_placeholder', '输入一句简单的话测试连接，例如：你好')" @keydown.enter="testModel"></CommonInput>
                        <button class="mt-[1.3rem] flex items-center justify-center bg-accent-special/70 hover:bg-accent-special hover:text-text-main text-text-dim px-6 py-2 rounded-lg font-bold transition-all" 
                          ...
                          {{ t('ui.settings.ai.start_test', '发起测试') }}
                        </button>
                      </div>
                      ...
                      <div v-if="testResponse" class="p-4 mt-2 rounded-xl text-text-soft bg-accent-special/10 border border-border-base/10 relative">
                        ...
                        <div class="text-xs text-text-dim mb-1 font-bold">{{ t('ui.settings.ai.response_result', 'AI 响应结果：') }}</div>
                        ...
                        <details v-if="testRawResponse" class="modal-section-subtle mt-3 p-3 text-xs text-text-dim">
                          <summary class="cursor-pointer font-bold text-text-main">{{ t('ui.settings.ai.view_raw_response', '查看原始返回内容') }}</summary>
...
const testPrompt = ref(t('ui.settings.ai.default_test_prompt', '介绍一下自己'))
...
const testModel = async () => {
  ...
  if (res?.ok) {
    ...
    toast.success(t('toast.settings.ai.test_success', '模型测试成功'))
    return
  }
  if (res?.isEmpty) {
    testResponse.value = t('ui.settings.ai.empty_response_help', '模型已返回，但内容为空。可尝试切换模型、检查代理兼容策略或改用正式助手会话测试。')
