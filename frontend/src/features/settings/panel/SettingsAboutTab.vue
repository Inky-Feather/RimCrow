<template>
  <section class="animate-in fade-in slide-in-from-right-4">
    <h3 class="mb-6 text-lg font-bold text-text-main">{{ t('ui.settings.about.title', '关于项目') }}</h3>
    ...
    <div class="space-y-4">
      <div class="modal-section p-4">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div class="shrink-0 rounded-xl lg:w-50">
            <div class="mb-2">
              <CommonSwitch :label="t('ui.settings.about.auto_update_check', '自动检查更新')" v-model="formData.enable_auto_update_check" :description="t('ui.settings.about.auto_update_check_desc', '开启后，软件启动时会自动提醒新版本。')" mini />
            </div>
            <div class="flex flex-wrap gap-2">
              <button type="button" @click="handleShowChangelog()" :disabled="isPending('changelog')" :class="isPending('changelog') ? 'app-action-disabled' : ''"
                ...
                {{ isPending('changelog') ? t('common.status.reading', '读取中') : t('ui.settings.about.changelog', '更新日志') }}
              </button>
              <button type="button" @click="handleCheckUpdate()" :disabled="appStore.updateState.isChecking"
                ...
                {{ appStore.updateState.isChecking ? t('common.status.checking', '检查中') : t('ui.settings.about.check_update', '检查更新') }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <div class="modal-section p-4">
          <div>
            <h4 class="text-sm font-bold text-text-main">{{ t('ui.settings.about.downloads', '下载地址') }}</h4>
            <p class="mt-1 text-xs leading-relaxed text-text-dim">{{ t('ui.settings.about.downloads_desc', '查看源码、发布包和备用下载地址。') }}</p>
          </div>
                ...
                <div class="min-w-0">
                  ...
                  <div class="flex gap-2 items-center mt-0.5 break-all select-text font-mono text-[0.72rem]" :class="item.primary ? 'text-accent-primary' : 'text-text-dim'">
                    ...
                    <div v-if="item.password" class="text-xs text-text-main select-none">
                      {{ t('ui.settings.about.password', '密码：') }}<span class="select-text font-mono text-text-dim">{{ item.password }}</span>
                    </div>
                  </div>
                  <div v-if="item.note" class="mt-0.5 text-xs leading-relaxed text-text-dim">{{ item.note }}</div>
                </div>
                <div class="flex shrink-0 gap-2">
                  <button type="button" @click="openExternalUrl(item.url)" class="about-icon-button" :title="t('common.action.open_link', '打开链接')">
        ...
        <div class="modal-section p-4">
          <div>
            <h4 class="text-sm font-bold text-text-main">{{ t('ui.settings.about.feedback', '反馈与交流') }}</h4>
            <p class="mt-1 text-xs leading-relaxed text-text-dim">{{ t('ui.settings.about.feedback_desc', '遇到问题时，建议带上截图、日志和复现步骤。') }}</p>
          </div>
          ...
          <div class="mt-3 space-y-2">
            <div v-for="item in feedbackItems" :key="item.label" class="border-t border-border-base/10 pt-2 first:border-t-0 first:pt-0">
              <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <div class="flex shrink-0 gap-2">
                  <button v-if="item.url" type="button" @click="openExternalUrl(item.url)" class="about-icon-button" :title="t('common.action.open_link', '打开链接')">
                    <ExternalLink class="size-3.5" />
                  </button>
        ...
        <div v-if="aboutMeta.references.length" class="modal-section p-4 xl:col-span-2">
          <div>
            <h4 class="text-sm font-bold text-text-main">{{ t('ui.settings.about.references', '参考项目') }}</h4>
            <p class="mt-1 text-xs leading-relaxed text-text-dim">{{ t('ui.settings.about.references_desc', '这些项目为排序、日志分析和管理功能提供了参考。') }}</p>
          </div>
        ...
        <div v-if="donate.enabled" class="flex gap-4 items-center modal-section p-4 xl:col-span-2">
          ...
          <div class="flex-1 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <button type="button" @click="showDonateModal = true"
              ...
              {{ t('ui.settings.about.view_donate_qr', '查看收款码') }}
            </button>
          </div>
        </div>

        <div v-if="aboutMeta.credits.length" class="modal-section p-4">
          <h4 class="text-sm font-bold text-text-main">{{ t('ui.settings.about.credits', '致谢') }}</h4>
          <div class="mt-3 space-y-2">
    ...
    <Teleport to="body">
      <Transition name="donate-modal-fade">
        <div v-if="isDonateModalVisible" class="fixed inset-0 z-130 flex items-center justify-center px-3 py-4 text-text-main selection:bg-bg-overlay/10">
          <button type="button" class="absolute inset-0 bg-overlay-scrim backdrop-blur-md" :aria-label="t('ui.settings.about.close_donate_modal', '关闭打赏弹窗')" @click="closeDonateModal"></button>
          ...
          <section class="modal-surface relative z-10 flex w-[min(28rem,calc(100vw-1.5rem))] max-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-2xl border-border-base/18"
            role="dialog" aria-modal="true" :aria-label="t('ui.settings.about.donate_title', '打赏支持')">
            ...
            <header class="modal-header relative z-10 flex shrink-0 items-start justify-between gap-4 px-5 py-4">
              <div class="min-w-0">
                <h2 class="truncate text-lg font-black tracking-wide text-text-main">{{ t('ui.settings.about.donate_title', '打赏支持') }}</h2>
                <p class="mt-1 text-xs leading-relaxed text-text-dim">{{ t('ui.settings.about.donate_desc', '感谢支持！微信和支付宝都可以扫 ~') }}</p>
              </div>
              <button type="button" class="modal-close-button" :aria-label="t('common.action.close', '关闭')" @click="closeDonateModal">
                <X class="size-4" />
              </button>
            </header>
            ...
            <div class="relative z-10 min-h-0 overflow-y-auto p-4 custom-scrollbar">
              <div class="w-full max-w-full flex flex-col gap-3">
                <div v-if="donate.qrImageUrl" class="rounded-xl border border-border-base/10 bg-bg-overlay/5 p-2">
                  <img :src="donate.qrImageUrl" :alt="t('ui.settings.about.donate_qr_alt', '打赏收款码')" class="block h-auto w-full max-h-[calc(100vh-12rem)] rounded-lg object-contain" />
                </div>
...
const DEFAULT_ABOUT_META = Object.freeze({
  ...
  donate: {
    enabled: false,
    title: t('ui.settings.about.donate_title', '打赏支持'),
    description: '',
    qrImageUrl: '',
  },
})
...
const normalizeAboutMeta = (rawMeta) => {
  ...
  return {
    ...
    donate: {
      ...
      title: String(rawMeta?.donate?.title || t('ui.settings.about.donate_title', '打赏支持')).trim(),
...
const downloadLinks = computed(() => {
  const links = [
    { label: t('ui.settings.about.github_repo', 'GitHub 仓库'), url: repositoryUrl.value, note: t('ui.settings.about.github_repo_note', '查看发布包、问题反馈和源码。'), primary: true },
    { label: t('ui.settings.about.github_releases', 'GitHub 发布页'), url: releasesUrl.value, note: t('ui.settings.about.github_releases_note', '下载 GitHub 发布包，或查看每个版本的发布内容。') },
  ].filter(item => item.url)
  if (lanzouSource.value) {
    links.push({
      label: t('ui.settings.about.lanzou_mirror', '蓝奏云分流'),
      url: lanzouSource.value.url,
      password: lanzouSource.value.password,
      note: t('ui.settings.about.lanzou_mirror_note', '网络不稳定时，可以尝试从这里下载备用包。'),
    })
  }
  return links
})
const feedbackItems = computed(() => [
  {
    label: t('ui.settings.about.issue_feedback', '问题反馈'),
    value: issuesUrl.value,
    url: issuesUrl.value,
    copyValue: issuesUrl.value,
    copyTitle: t('common.action.copy_url', '复制地址'),
    note: t('ui.settings.about.issue_feedback_note', '适合记录需要跟进的问题和建议。'),
    primary: true,
  },
  {
  ...
  {
    label: t('ui.settings.about.tieba_thread', '贴吧主贴'),
    value: aboutMeta.value.feedback.tiebaUrl,
    url: aboutMeta.value.feedback.tiebaUrl,
    copyValue: aboutMeta.value.feedback.tiebaUrl,
    copyTitle: t('common.action.copy_url', '复制地址'),
    note: aboutMeta.value.feedback.tiebaDescription || t('ui.settings.about.tieba_thread_note', '查看集中说明和公开讨论。'),
  },
...
const openExternalUrl = async (url) => {
  ...
  try {
    ...
    toast.error(t('toast.settings.about.open_failed', '打开失败，请手动复制链接。'))
    return
  }
...
const copyText = async (text) => {
  ...
  try {
    ...
    toast.success(t('toast.settings.about.copied', '已复制：{text}', { text: normalizedText }), { timeout: 600 })
  } catch {
    toast.error(t('toast.settings.about.copy_failed', '复制失败，请手动复制。'))
  }
}
