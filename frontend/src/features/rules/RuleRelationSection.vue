<template>
  <div class="min-h-20 m-1 pb-2 rounded-lg relative transition-colors duration-200" :class="`bg-accent-${color}/10`">
    ...
    <div class="flex items-center text-xs text-text-dim px-2 py-1 gap-1">
      ...
      <span class="text-accent-tip">{{ t('ui.rule_relation.native_rules', '原始规则 ({count})', { count: nativeRules.length || 0 }) }}</span>
    ...
    <div class="flex items-center text-xs text-text-dim px-2 py-1 gap-1">
      ...
      <span class="text-accent-cool">{{ t('ui.rule_relation.community_rules', '社区规则 ({count})', { count: Object.keys(communityRules).length || 0 }) }}</span>
    ...
    <template v-if="showWorkshop">
      <div class="flex items-center text-xs text-text-dim px-2 py-1 gap-1">
        ...
        <span class="text-accent-warning">{{ t('ui.rule_relation.workshop_rules', '工坊规则 ({count})', { count: Object.keys(workshopRules).length || 0 }) }}</span>
    ...
    <div class="flex items-center text-xs text-text-dim px-2 py-1 gap-1">
      ...
      <span class="text-accent-special">{{ t('ui.rule_relation.user_rules', '用户规则 ({count})', { count: userRules.length || 0 }) }}</span>
          ...
          <div class="relative group">
            ...
            <div class="absolute right-1 top-1/2 -translate-y-1/2 mr-1 overflow-visible gap-1 group text-sm font-medium flex flex-row-reverse items-center rtl:space-x-reverse">
              <button @click.stop="$emit('edit-comment', ruleType, dataKey, $event)" class="group z-50 h-5 px-2.5 relative rounded-md whitespace-nowrap cursor-pointer
                ...
                v-tooltip="record.comment || t('tooltip.rule_relation.add_comment', '[[__点击添加说明__]]')">
              ...
              <button @click.stop="$emit('remove-user-rule', ruleType, dataKey, $event)" class="w-0 h-0 px-1 translate-x-3 opacity-0 overflow-hidden rounded-md whitespace-nowrap cursor-pointer
                inline-flex items-center self-center justify-center justify-self-center tracking-wide transition-all duration-300
