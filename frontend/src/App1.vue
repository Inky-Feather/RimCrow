  ...
  <div class="h-dvh w-screen flex flex-col p-1 overflow-hidden font-sans bg-bg-deep text-text-main select-none">
    ...
    <div ref="containerRef" class="flex flex-1 w-full overflow-hidden relative ">
      ...
      <div class="h-full p-1 overflow-hidden relative transition-opacity"
          ...
          <div class="h-full rounded-2xl pb-2 overflow-hidden bg-bg-surface/40 backdrop-blur-sm border border-white/5 shadow-2xl">
            <ModDetails />
          </div>
      </div>
      ...
      <div class="h-full p-1 overflow-hidden relative transition-opacity"
          ...
          <div class="h-full rounded-2xl overflow-hidden bg-bg-surface/40 backdrop-blur-sm border border-white/5 shadow-2xl">
            ...
            <ModList v-model="store.inactiveIds" variant="inactive" listColor="primary" />
      ...
      <div class="h-full p-1 overflow-hidden relative transition-opacity"
          ...
          <div class="h-full rounded-2xl overflow-hidden bg-bg-surface/40 backdrop-blur-sm border border-white/5 shadow-2xl">
            ...
            <section class=" flex flex-col bg-gray-850 border-r border-gray-800 min-w-[280px]">
              <div class="flex-1 overflow-y-auto p-2 custom-scrollbar" ref="availableListContainer">
                  <div ref="availableListRef" 
                      key="`available-2`"
                      class="space-y-1 min-h-[100px] pb-10" 
                      data-list-type="available">
                      <div v-for="(mod, index) in renderList" :key="mod.package_id"
                          class="mod-item mod-grid p-2 rounded bg-gray-800 hover:bg-gray-750 border border-gray-700/50 cursor-pointer select-none group relative"
                          :class="{'selected': isSelected(mod.package_id)}"
                          @mousedown="handleMouseDown(mod, $event)"
                          @click="handleClick(mod, $event)">
                          
                          <!-- 序号 -->
                          <div class="text-[10px] font-mono text-gray-600 text-center">{{ index + 1 }}</div>
                          
                          <!-- 缩略图 -->
                          <img v-if="!mod.is_missing" :src="store.getAssetUrl(mod.package_id)" class="w-8 h-8 rounded object-cover bg-black/20 opacity-80 group-hover:opacity-100">
                          
                          <!-- 标题区 -->
                          <div class="min-w-0 flex flex-col justify-center">
                              <span class="text-[10px] text-gray-500 leading-none mb-0.5 truncate">{{ mod.name }}</span>
                              <span class="text-xs font-bold text-gray-200 leading-tight truncate">{{ mod.alias || mod.name }}</span>
                          </div>
            ...
            <!-- <ModList v-model="store.activeIds" variant="active" listColor="success" /> -->
      ...
      <div class="h-full p-1 flex flex-col overflow-hidden relative transition-opacity"
          ...
          <div class="flex-1 rounded-t-2xl overflow-hidden bg-bg-surface/40 backdrop-blur-sm border border-white/5 ">
            ...
            <ModList v-if="activeTab === 'Temp'" v-model="store.tempIds" variant="temp" listColor="warning"/>
...
const renderList = computed(() => {
  if (!useModStore()) return []
  return useModStore().inactiveIds.map(id => store.getModById(id))
})
const selectedModIds = useModStore().selectedMods
const isSelected = (id) => selectedModIds.value.has(id);

const handleMouseDown = (mod, event) => {
    if (isSelected(mod.package_id) && !event.ctrlKey && !event.shiftKey) {
        isMouseDownOnSelected.value = true;
    } else {
        isMouseDownOnSelected.value = false;
        handleSelect(mod, event);
    }
};
const handleClick = (mod, event) => {
    if (isMouseDownOnSelected.value) {
        handleSelect(mod, event);
        isMouseDownOnSelected.value = false;
    }
};

const handleSelect = (mod, event) => {
    const id = mod.package_id;
    lastSelectedMod.value = mod;
    
    if (event.ctrlKey || event.metaKey) {
        if (selectedModIds.value.has(id)) selectedModIds.value.delete(id);
        else selectedModIds.value.add(id);
        shiftAnchorId.value = id;
    } else if (event.shiftKey && shiftAnchorId.value) {
        const listType = isModActive(id) ? 'active' : 'available';
        const list = listType === renderList.value;
        const idx1 = list.findIndex(m => m.package_id === shiftAnchorId.value);
        const idx2 = list.findIndex(m => m.package_id === id);
        if (idx1 !== -1 && idx2 !== -1) {
            const start = Math.min(idx1, idx2);
            const end = Math.max(idx1, idx2);
            selectedModIds.value.clear();
            for (let i = start; i <= end; i++) selectedModIds.value.add(list[i].package_id);
        }
    } else {
        selectedModIds.value.clear();
