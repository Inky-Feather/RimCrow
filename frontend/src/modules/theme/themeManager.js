...
export const THEME_TOKEN_GROUPS = [
  { key: 'bg', label: '背景色', tokens: [
    ...
    { key: 'highlight', label: '层级背景色', usage: '影响悬停、选中和更突出的区域。' },
  ] },
...
export const applyTheme = (theme) => {
  ...
  const { bg, glass, accent, text } = normalized.tokens
  const isLightTheme = getRelativeLuminance(bg?.deep) > 0.55

  Object.entries(bg || {}).forEach(([key, value]) => setCssVar(root, `--color-bg-${key}`, toTailwindColorValue(value)))
  ...
  Object.entries(text || {}).forEach(([key, value]) => setCssVar(root, `--color-text-${key}`, toTailwindColorValue(value)))
  ...
  setCssVar(root, '--primary-foreground', isLightTheme ? '#ffffff' : '#000000')
  setCssVar(root, '--secondary', toTailwindColorValue(bg?.highlight))
  setCssVar(root, '--secondary-foreground', toTailwindColorValue(text?.main))
  setCssVar(root, '--muted', toTailwindColorValue(bg?.highlight))
  ...
  setCssVar(root, '--border', `rgba(${colorToRgbComponents(text?.main)}, 0.16)`)
  setCssVar(root, '--input', `rgba(${colorToRgbComponents(text?.main)}, 0.18)`)
